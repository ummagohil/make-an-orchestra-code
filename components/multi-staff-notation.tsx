...
interface MultiStaffNotationProps {
  ...
  currentStep: number
  isPlaying: boolean
  onAddNote: (trackId: string, step: number, pitch: string, duration: NoteDuration) => void
  keySignature: string
  scale: string
  totalSteps: number
...
export default function MultiStaffNotation({
  ...
  const [hoveredPosition, setHoveredPosition] = useState<{ trackId: string; step: number; pitch: string } | null>(null)
  const [staffDimensions, setStaffDimensions] = useState<Record<string, { width: number; height: number }>>({})
  ...
  const handleDrop = (e: React.DragEvent, trackId: string) => {
    ...
    const duration = (e.dataTransfer.getData("noteDuration") as NoteDuration) || activeDuration
    if (!duration) return
    ...
    if (clef === "treble") {
      ...
      const pitchIndex = Math.min(Math.max(0, pitchPosition), TREBLE_CLEF_PITCHES.length - 1)
      ...
      const pitchIndex = Math.min(Math.max(0, pitchPosition), BASS_CLEF_PITCHES.length - 1)
  ...
  const handleNoteClick = (trackId: string, step: number, pitch: string) => {
    onAddNote(trackId, step, pitch) // This will toggle the note off
  }
  ...
  const renderNoteSymbol = (duration: NoteDuration, stemDirection: "up" | "down") => {
    if (duration === "whole") {
      return (
      ...
      if (stemDirection === "up") {
        ...
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
            <ellipse cx="14" cy="8" rx="6" ry="4" stroke="black" strokeWidth="1" fill="white" />
      ...
      if (stemDirection === "up") {
        ...
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
            <ellipse cx="14" cy="8" rx="6" ry="4" stroke="black" strokeWidth="1" fill="black" />
            <line x1="8" y1="8" x2="8" y2="20" stroke="black" strokeWidth="1" />
          </svg>
        )
      }
      ...
      if (stemDirection === "up") {
        ...
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
            <ellipse cx="14" cy="8" rx="6" ry="4" stroke="black" strokeWidth="1" fill="black" />
            <line x1="8" y1="8" x2="8" y2="20" stroke="black" strokeWidth="1" />
            <path d="M8,20 C5,18 2,16 2,14" stroke="black" strokeWidth="1" fill="none" />
          </svg>
        )
      }
      ...
      if (stemDirection === "up") {
        ...
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
            ...
            <line x1="8" y1="8" x2="8" y2="20" stroke="black" strokeWidth="1" />
            <path d="M8,20 C5,18 2,16 2,14" stroke="black" strokeWidth="1" fill="none" />
            <path d="M8,16 C5,14 2,12 2,10" stroke="black" strokeWidth="1" fill="none" />
          </svg>
        )
      }
    }
    ...
    return (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
        <ellipse cx="12" cy="12" rx="6" ry="4" stroke="black" strokeWidth="1" fill="black" />
  ...
  const renderTrebleClef = () => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 95.116 153.12" width="30" height="80">
      <path
  ...
  const renderBassClef = () => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 40" width="30" height="40">
      <path
    ...
    return (
      <div className="absolute left-10 top-[15px] h-[90px]">
        {sharps.map((note, index) => {
          const position = sharpPositions[clef][note as keyof typeof sharpPositions.treble]
          ...
          return (
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 10 24" width="10" height="24">
                <path d="M1,0 L1,24 M9,6 L9,18 M1,12 L9,8 M1,16 L9,12" stroke="black" strokeWidth="1.5" fill="none" />
              </svg>
            </div>
          )
        })}
        ...
        {flats.map((note, index) => {
          const position = flatPositions[clef][note as keyof typeof flatPositions.treble]
          const top = 60 - position * 7.5
  ...
  const renderTimeSignature = () => (
    <div className="absolute left-[60px] top-[30px] h-[60px] flex flex-col items-center justify-center">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 60" width="20" height="60">
        <text x="10" y="25" fontSize="24" fontFamily="serif" textAnchor="middle" dominantBaseline="middle">
          4
        </text>
        <text x="10" y="55" fontSize="24" fontFamily="serif" textAnchor="middle" dominantBaseline="middle">
          4
        </text>
      </svg>
    </div>
          ...
          return (
            <div
              ...
              className={`p-4 rounded-md border transition-colors ${
                isCurrentTrack ? "border-primary/50 bg-primary/5" : "hover:bg-muted/50"
              }`}
              onClick={() => onSelectTrack(track.id)}
            >
              <div className="flex items-center mb-2">
                <h3 className="text-lg font-medium capitalize">{track.instrument}</h3>
                <span className="ml-2 text-xs bg-muted px-2 py-0.5 rounded">{clef} clef</span>
                {track.muted && <span className="ml-2 text-xs bg-muted px-2 py-0.5 rounded">Muted</span>}
              </div>

              <div
                className="relative w-full h-[180px] bg-white dark:bg-slate-900 border rounded-md overflow-hidden"
                ref={(el) => (staffRefs.current[track.id] = el)}
              ...
              >
                  ...
                  {Array.from({ length: totalSteps + 1 }, (_, i) => (
                    <div
                      ...
                      className={`absolute h-[90px] w-[1px] ${
                        i % 4 === 0 ? "bg-gray-500 dark:bg-gray-500" : "bg-gray-300 dark:bg-gray-700"
                      }`}
                      style={{
                ...
                {/* Notes */}
                {track.notes.map((note) => renderNote(note, track.id, currentStep === note.step && isPlaying))}
